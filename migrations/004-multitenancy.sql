...
--   * привязывает существующие данные первого клуба к его записи;
...
--  он остаётся у первого клуба как есть, у новых клубов вкладка скрыта.
-- ============================================================

-- Фиксированный uuid первого (существующего) клуба, чтобы бэкфилл был
-- детерминированным (произвольное, но постоянное значение).
--   первый клуб id = 11111111-1111-1111-1111-111111111111
...
-- Первый (существующий) клуб — первый арендатор. Владелец — тот же tg-id,
-- что и в staff. Имя берём из существующих настроек (не по id — колонка id
-- ниже удаляется, иначе повторный запуск сломался бы на этой строке).
insert into clubs (id, code, name, owner_tg_id, plan, status, booking_enabled)
values ('11111111-1111-1111-1111-111111111111', 'club01',
        coalesce((select club_name from settings limit 1), 'Клуб'),
        420115296, 'paid', 'active', true)
on conflict (id) do nothing;


-- ------------------------------------------------------------
--  2. Колонки club_id (nullable), затем бэкфилл = первый клуб
-- ------------------------------------------------------------
