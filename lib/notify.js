...
function reminderText(item, grace, settings) {
  return (
    ...
    "🎁 " + item.title + "\n" +
    "Код: " + item.code + "\n\n" +
    "Забрать можно до " + formatMoment(item.expiresAt, settings.timezone) + ".\n" +
    "Мы добавили ещё " + grace + " минут к сроку, чтобы ты успел зайти.\n\n" +
    "Покажи код на стойке " + (settings.club_name || "клуба") + "."
  );
}

// Клиенту нужно понятное «сегодня в 21:30», а не ISO-строка. Часовой
// пояс берём из настроек клуба: сервер живёт в UTC, и без этого время
// сгорания уехало бы на несколько часов.
function formatMoment(iso, timezone) {
  ...
  try {
    ...
    // Неизвестный часовой пояс не должен ронять рассылку.
...
async function sendBroadcastBatch(broadcastId, size) {
  ...
  for (const userId of ids) {
    const res = await tg.sendMessage(userId, batch.text, markup);
    results.push({
