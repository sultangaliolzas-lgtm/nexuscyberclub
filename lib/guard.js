...
// Оператор платформы (SaaS): id из OWNER_IDS/STAFF_IDS. Ему доступен
// список всех подключённых клубов. Обычные владельцы клубов сюда не входят.
function isPlatformOwner(userId) {
  return ENV_OWNER_IDS.includes(Number(userId));
}
