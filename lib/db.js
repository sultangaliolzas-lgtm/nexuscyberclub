...
// Помечает клиентов, которым бот писать не может: закрыли переписку,
// удалили аккаунт или никогда не нажимали Start. Без этого каждая
// рассылка тратила бы на них попытку и портила статистику доставки.
async function setCanMessage(ids, value) {
  const list = (Array.isArray(ids) ? ids : [ids]).map(Number).filter(function (n) {
    return isFinite(n) && n > 0;
  });
  if (!list.length) return null;

  return request("users?id=in.(" + list.join(",") + ")", {
    method: "PATCH",
