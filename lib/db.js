...
// Длина бесплатной пробы (дней). Один источник правды для панели оператора
// (показ «осталось N дней») и для автозаморозки в кроне.
const TRIAL_DAYS = 7;
...
// Все клубы платформы — для панели оператора «кто подключился». Счётчики
// клиентов и выданных призов подтягиваем встроенным агрегатом PostgREST
// по внешним ключам (users.club_id, inventory.club_id) — одним запросом.
// Если агрегат почему-то недоступен, отдаём клубы без счётчиков, чтобы
// список «кто подключился» работал в любом случае.
async function listAllClubs() {
  const base = "clubs?select=code,name,owner_tg_id,plan,status,created_at,paid_until,booking_enabled,is_default";
  const order = "&order=created_at.desc";
  try {
    return (await request(base + ",users(count),inventory(count)" + order, { method: "GET" })) || [];
  } catch (e) {
    return (await request(base + order, { method: "GET" })) || [];
  }
}

// Полное удаление клуба со всеми его данными. Дочерние строки удаляем
// раньше родительских (порядок соблюдает внешние ключи), сам клуб — в
// конце. Идемпотентно: повторный вызов по уже удалённому клубу безвреден.
async function deleteClub(clubId) {
  const id = encodeURIComponent(clubId);
  const children = [
    "broadcast_recipients", "broadcasts", "inventory", "events",
    "config_log", "prizes", "staff", "settings", "users"
  ];
  for (const t of children) {
    await request(t + "?club_id=eq." + id, { method: "DELETE", headers: { Prefer: "return=minimal" } });
  }
  await request("clubs?id=eq." + id, { method: "DELETE", headers: { Prefer: "return=minimal" } });
}

// Активация/продление клуба вручную оператором: снимаем заморозку и даём
// оплаченный доступ на N дней вперёд (по умолчанию неделя). Продлеваем от
// большего из «сейчас» и текущего paid_until, чтобы дни не сгорали. До
// появления биллинга Tribute это ручная замена оплаты.
async function activateClub(clubId, days) {
  const add = (Number(days) > 0 ? Number(days) : TRIAL_DAYS) * 86400000;
  const club = await getClubById(clubId);
  const cur = club && club.paid_until ? new Date(club.paid_until).getTime() : 0;
  const base = cur > Date.now() ? cur : Date.now();
  const until = new Date(base + add).toISOString();

  return request("clubs?id=eq." + encodeURIComponent(clubId), {
    method: "PATCH",
    headers: { Prefer: "return=representation" },
    body: JSON.stringify({ status: "active", plan: "paid", paid_until: until })
  });
}

// Автозаморозка: пробные клубы с истёкшей неделей и платные с прошедшим
// сроком оплаты переводим в status='frozen'. Дефолтный (первый) клуб не
// трогаем. Возвращает, сколько заморожено, — для лога крона.
async function freezeExpiredClubs() {
  const trialCutoff = new Date(Date.now() - TRIAL_DAYS * 86400000).toISOString();
  const nowIso = new Date().toISOString();

  function freeze(filter) {
    return request("clubs?" + filter, {
      method: "PATCH",
      headers: { Prefer: "return=representation" },
  ...
  const trial = await freeze(
    "status=eq.active&plan=eq.trial&is_default=is.false&created_at=lt." + encodeURIComponent(trialCutoff)
  );
  const paid = await freeze(
    "status=eq.active&plan=eq.paid&is_default=is.false&paid_until=lt." + encodeURIComponent(nowIso)
  );

  return { trialFrozen: (trial || []).length, paidFrozen: (paid || []).length };
}

// Клуб по умолчанию — на переходный период, пока старые ссылки/QR первого
// клуба не содержат кода клуба. Берём из env DEFAULT_CLUB_CODE, а если он
// не задан и клуб в базе ровно один — считаем дефолтным его. Когда клубов
// становится несколько и дефолт не задан, возвращаем null (общий бот тогда
// показывает онбординг для входа без кода).
async function getFallbackClub() {
  const code = process.env.DEFAULT_CLUB_CODE;
  if (code) {
    const c = await getClubByCode(code);
    if (c) return c;
  }
  // Клуб, помеченный дефолтным (колонка появляется миграцией 005; до неё
  // запрос упадёт — тогда просто переходим к «единственному клубу»).
  try {
    const flagged = await request("clubs?is_default=is.true&select=*&limit=1", { method: "GET" });
    if (flagged && flagged[0]) return flagged[0];
  } catch (e) { /* колонки is_default ещё нет */ }
  // Иначе: если клуб в базе ровно один — считаем дефолтным его.
  const rows = await request("clubs?select=*&limit=2", { method: "GET" });
