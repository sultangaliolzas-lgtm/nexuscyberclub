-- ============================================================
--  Клубная рулетка — схема базы данных (одноклубная база v1)
...
--
--  ⚠️  МУЛЬТИАРЕНДНОСТЬ (много клубов) добавляется отдельно —
--      migrations/004-multitenancy.sql. Порядок для базы с нуля:
--      сначала этот файл, затем миграции по порядку (002, 003, 004).
--      НЕ выполняйте этот файл повторно ПОСЛЕ 004: он вернёт
--      одноклубные версии функций (do_spin(bigint) и т.п.) рядом с
--      мультиарендными, и PostgREST не сможет выбрать между ними.
--      Все правки после перехода — только через миграции.
-- ============================================================
